...
class S3DIS(Dataset):
    def __init__(self, root, area_nums, split='train', npoints=4096, r_prob=0.25):
        ...
        for fp in self.data_paths:
            area, space = os.path.split(os.path.split(fp)[0])
            space_id = os.path.join(area, '_'.join(space.split('_')[:2])) + '_'
    ...
    def get_random_partitioned_space(self):
        ...
        print("All space IDs:", self.space_ids)
        print("All data paths:", self.data_paths)

        # get random space id
        idx = random.randint(0, len(self.space_ids) - 1)
        space_id = self.space_ids[idx]
        print("Selected space_id:", space_id)

        if not space_id.endswith('/'):
            space_id = space_id.rstrip('_') + '/'
        ...
        print("Number of space paths found:", len(space_paths))
        ...
        if len(space_paths) == 0:
            print(f"No paths found for space_id {space_id}.")
            return torch.empty((0, self.npoints, 3)), torch.empty((0, self.npoints))
        ...
        for i, space_path in enumerate(space_paths):
            print(f"Reading space data from: {space_path}")
            ...
            print(f"Shape of loaded points: {_points.shape}")
            print(f"Shape of loaded targets: {_targets.shape}")

            # downsample point cloud
            _points, _targets = self.downsample(_points, _targets)
            print(f"Shape of downsampled points: {_points.shape}")
            print(f"Shape of downsampled targets: {_targets.shape}")

            # add points and targets to batch arrays
            points[i] = _points
            targets[i] = _targets

            print("Points tensor shape before return:", points.shape)
            print("Targets tensor shape before return:", targets.shape)
